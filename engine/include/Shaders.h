...
const char fs_shader[] =
    ...
    "   if (cosAngle < 0.94 && cosAngle >= 0.92) lightFactor = max(smoothstep(0.92, 0.94, cosAngle), pow(0.3, 4)); /* degrees [19 : 22] */"
    "   if (cosAngle < 0.92) lightFactor = pow(0.3 * cosAngle / 0.92, 4); /* over 24 degrees */"
    "   float radius = 1000.0;"
    ...
    "	float3 finalColor = attenuation * saturate(diffuseLighting.xyz * 0.6  + lightFactor * specular * 0.2);\n"
